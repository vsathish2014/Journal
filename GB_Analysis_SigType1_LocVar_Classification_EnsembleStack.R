...
#for ( lastDay in dayList){  
  ...
  locVar <- 3 # 3 for onsidering local variation 3 days
...
model_list[[i]] <- caretList(
  Class~., data=sample_data_ST1_Train_2[[i]],
...
library("caTools")
model_preds[[i]] <- lapply(model_list[[i]], predict, newdata=x_test_ST1[[i]], type="prob")
...
confusionMatrix(model_preds2[[1]] , as.factor(y_test_ST1[[1]]))
 
library(plyr)
accuracy_all <- ldply(accuracy, data.frame)

setwd("C:/IIITD/WIP/Analysis/Journal/TestResults")
write.csv(accuracy_all,"C:/IIITD/WIP/Analysis/Journal/TestResults/accuracy_all_SigType1_locvar3.csv" )
