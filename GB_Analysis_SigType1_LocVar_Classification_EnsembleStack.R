...
model_preds2_woP[[i]]$ensemble <- predict(glm_ensemble[[i]], newdata=x_test_ST1[[i]] )
...
Accuracy_ST1 <- list()
for (i in 1: noSamples) {
cm_ST1[[i]] <- confusionMatrix(model_preds2_woP[[i]]$ensemble , as.factor(y_test_ST1[[i]]))
misClasificError_ST1[[i]] <- mean(model_preds2_woP[[i]]$ensemble != y_test_ST1[[i]])
...
#   model_preds3[[i]]$ensemble <- predict(greedy_ensemble[[i]], newdata=x_test_ST1[[i]], type="prob")
#   CF_G <- coef(glm_ensemble[[i]]$ens_model$finalModel)[-1]
#   accuracy_Greedy[[i]] <- colAUC(model_preds2[[i]], y_test_ST1[[i]])
# }
...
model_preds2_all <-cbind( model_preds2_all, y_test_ST1_all)


model_preds2_woP_all <- ldply(model_preds2_woP, data.frame)
y_test_ST1_all <- ldply(y_test_ST1, data.frame)
model_preds2_woP_all <-cbind( model_preds2_woP_all, y_test_ST1_all)

accuracy_all <- ldply(accuracy, data.frame)


write.csv(accuracy_all,"C:/IIITD/WIP/Analysis/Journal/TestResults/accuracy_all_SigType1_PCA_locvar0_noRandom.csv" )

write.csv(model_preds2_all,"C:/IIITD/WIP/Analysis/Journal/TestResults/model_preds2_all_SigType1_PCA_locvar0_noRandom.csv" )

write.csv(model_preds2_woP_all,"C:/IIITD/WIP/Analysis/Journal/TestResults/model_preds2_woP_all_SigType1_PCA_locvar0_noRandom.csv" )
