...
#for ( lastDay in dayList){  
  ...
  locVar <- 5 # 3 for onsidering local variation 3 days
...
for (i in 1:noSamples){
  ...
  sample_data_ST2_Test_4[[i]]$Class <- ifelse(sample_data_ST2_Test_4[[i]]$Class=="F",1,0)
}
...
train_predictions_prob_ST1 <- list()
cm_ST1 <- list()
...
train_predictions_prob_ST2 <- list()
cm_ST2 <- list()
...
for (i in 1: noSamples ){
  ...
  train_predictions_prob_ST1[[i]] <-  predict(glmModel_ST1[[i]], sample_data_ST1_Train_4[[i]] )
  ...
  train_predictions_prob_ST2[[i]] <-  predict(glmModel_ST2[[i]], sample_data_ST2_Train_4[[i]] )
...
fileTrainPredAct <- paste0("C:/IIITD/WIP/Analysis/Journal/TestResults_Final/","Train_Pred_Act", "_dbf_",dbf,"_locVar",locVar,    "_PCA_LR.csv")
filePredAct <- paste0("C:/IIITD/WIP/Analysis/Journal/TestResults_Final/","Pred_Act", "_dbf_",dbf,"_locVar",locVar,"_PCA_LR.csv")
filePredAct1 <- paste0("C:/IIITD/WIP/Analysis/Journal/TestResults_Final/","Pred_Act", "_dbf_",dbf,"_locVar",locVar,"_PCA_LR_test.csv")


trainpred_act <- data.frame(cbind(1, sample_data_ST1_Train_4[[1]]$Class,train_predictions_prob_ST1[[1]],train_predictions_prob_ST2[[1]]))
write.table(trainpred_act,filePredAct,sep = ",", append = T,col.names = T)

for (i in 2:noSamples){
  trainpred_act <- data.frame(cbind(i, sample_data_ST1_Train_4[[i]]$Class,train_predictions_prob_ST1[[i]],train_predictions_prob_ST2[[i]]))
  write.table(trainpred_act,fileTrainPredAct, sep = ",",append = T,col.names = F)
}
...
fileCM_ST1 <- paste0("C:/IIITD/WIP/Analysis/Journal/TestResults_Final/","CM_","SigType_1","_dbf_",dbf,"_locVar",locVar,"_PCA_LR.csv")
fileCM_ST2 <- paste0("C:/IIITD/WIP/Analysis/Journal/TestResults_Final/","CM_","SigType_2","_dbf_",dbf,"_locVar",locVar,"_PCA_LR.csv")
